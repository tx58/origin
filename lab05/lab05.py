...
def pigify(w):
    ...
    2.  begins with a vowel, append 'hay' to the end of the word
    3.  starts with 'q', move 'qu' to the end of the word, and append 'ay'.
    4.  begins with a consonant, move all the consonants up to the first vowel (if any) to the end and add 'ay'.
    ...
    if first_vowel(w)==0:
        output= w+'hay'
    if (first_vowel(w)!=0 and w[0]!="q"):
        if first_vowel(w[1:])==-1:
            output= w+'ay'
        else:
            a=first_vowel(w[1:])
            output= w[a+1:]+ w[:a+1]+'ay'
    if (first_vowel(w)!=0 and w[0]=="q"):
        output= w[2:]+w[:2]+'ay'
    return output
...
def add_time(time1, time2):
    ...
    if (time1.minutes+ time2.minutes > 59):
        output= Time(time1.hours+time2.hours+1, time1.minutes+time2.minutes-60)
    else:
        output= Time(time1.hours+time2.hours, time1.minutes+time2.minutes)
    return output
    pass # STUB. Implement me
